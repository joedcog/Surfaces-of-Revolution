...
for (var i = 0; i <= size * (b - a); i++) {
    tempYArr.push(evaluateEquation(xVal, equationToEval));
    xVal = xVal - (1 / size);
}
function integral() {
...
function surfaceArea() {
  ...
  var tempY = 0;
  var prevYVal = 0;
  var count = size * (b - a) - 1;
  for (var i = 0; i < size * (b - a) - 1; i++) {
    tempY = (((tempYArr[count] + evaluateEquation(xVal - (1 / size), equationToEval)) / 2) * (1 / size));
    // integralValue += parseFloat(tempY.toFixed(6));
    // xVal = xVal - (1 / size);
    if (i > 0) {
      if (!isFinite(tempY)) {
